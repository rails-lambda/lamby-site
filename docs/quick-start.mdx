...
First, configure the AWS CLI with your AWS access key and secret. This is only used for deploying to AWS and is not shipped in a production container. For the security-minded, please see our full <DocLink id="anatomy" anchor="deployment--cicd" /> deployment section.
