...
Based off the current state of our [cookiecutter](https://github.com/rails-lambda/lamby-cookiecutter) project, here are the changes required to switch to a `x86_64` deployment target. First, change your CircleCI workflows default machine from `arm.large`` to a standard large.
